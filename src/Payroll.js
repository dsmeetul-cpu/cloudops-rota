...
function PayrollReports({ users, timesheets, incidents, upgrades, overtime, toil, rota, holidays,
  ...
  const cycleData = useMemo(() => visUsers.map(u => ({ u, ...getUserData(u, selCycleStart, selCycleEnd) })),
  ...
  const trendCycles = useMemo(() => allCycles.slice(0,4).reverse(), [allCycles]);
  const trendData   = useMemo(() => trendCycles.map(c => {
  ...
  const tbl   = { width:'100%', fontSize:12, borderCollapse:'collapse' };
  ...
  const dot   = bg => ({ width:10, height:10, borderRadius:2, background:bg, display:'inline-block', marginRight:4, flexShrink:0 });
  const pill  = (bg,fg) => ({ display:'inline-block', fontSize:10, padding:'2px 7px', borderRadius:10, background:bg, color:fg, fontWeight:600 });
  ...
  const ChartCard = ({ title, sub, legend, height, id, children }) => (
    <div style={card}>
      ...
      <div style={{ position:'relative', height: height||220 }}>
        <canvas id={id} role="img" aria-label={title}>{children}</canvas>
      </div>
    </div>
  );
                  ...
                  <React.Fragment key={u.id}>
                    ...
                    <tr>
                      <td colSpan={8} style={{ padding:'2px 8px 10px 44px', borderBottom:'1px solid rgba(148,163,184,0.08)', fontSize:11 }}>
                        {[incHrs>0 && <span key="i" style={pill('rgba(252,211,77,0.15)','#fcd34d')}>{incHrs}h incidents</span>,
                          overtimeHrs>0 && <span key="o" style={pill('rgba(244,114,182,0.15)','#f472b6')}>{overtimeHrs}h overtime</span>,
                          (tb?.balance||0)>0 && <span key="t" style={pill('rgba(56,189,248,0.15)','#38bdf8')}>{tb.balance}h TOIL</span>,
                          upgradeHrs>0 && <span key="u" style={pill('rgba(110,231,183,0.15)','#6ee7b7')}>{upgradeHrs}h upgrades</span>,
                          bankHolHrs>0 && <span key="b" style={pill('rgba(252,165,165,0.15)','#fca5a5')}>{bankHolHrs}h bank hol</span>,
                        ].filter(Boolean).map((el,i,a)=>[el, i<a.length-1?<span key={`s${i}`}> </span>:null]).flat().filter(Boolean)}
  ...
  const renderTrend = () => (
    <>
      ...
      <ChartCard title={`Hours trend — last ${trendData.length} cycles`} sub="Standby, incidents and overtime" id="rpt-trend" height={260}
