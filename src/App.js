...
// ── calcOncallPay / calcTOILBalance ────────────────────────────────────────
// Moved to src/Payroll.js and re-exported. Imported at top of this file via:
//   import Payroll, { calcOncallPay, calcTOILBalance } from './Payroll';
...
function Capacity({ users, rota, holidays, timesheets, incidents, isManager }) {
  ...
  const engineerCapacity = users.map(u => {
    ...
    // ── FIX: incident hours calc ──────────────────────────────────────────
    // Old code used (weekday_oncall + weekend_oncall) — Incidents.js now writes
    // to worked_wd / worked_we (fixing a double-count bug), so those fields are 0.
    // Also the old date filter excluded all INC entries because "INC MBA47-xxx"
    // always fails w <= endDate string comparison ("I" > "2" in ASCII).
    const incHrs = sheets
      .filter(e => {
        if (!e.week || !e.week.startsWith('INC')) return false;
        if (!liveIds.has(e.week.slice(4).trim())) return false;
        return true;
      })
      .reduce((a, e) => {
